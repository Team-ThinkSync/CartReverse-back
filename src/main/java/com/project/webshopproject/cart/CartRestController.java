...
public class CartRestController {
    ...
    // 장바구니 전체 조회
    @GetMapping("/carts")
    public ResponseEntity<RestApiResponseDto<Page<CartResponseDto>>> getCart(
            @AuthenticationPrincipal final UserDetailsImpl userDetails,
            @RequestParam(defaultValue = "1") int page
    ) {
        try {
            Pageable pageable = PageRequest.of(page - 1, 10);
            Page<CartResponseDto> cartItems = cartService.getAllCartItem(userDetails.getEmail(), pageable);
